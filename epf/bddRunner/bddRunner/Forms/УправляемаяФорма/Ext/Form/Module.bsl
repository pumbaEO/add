...
Процедура КаталогПроектаПриИзменении(Элемент)
	ЗагрузитьПеременныеКонтекстаИзФайлаНастроек(Объект.КаталогПроекта);
КонецПроцедуры
...
Процедура КаталогВыборЗавершение(ВыбранныеФайлы, ПараметрыОбъекта) Экспорт
	...
	ЗагрузитьПеременныеКонтекстаИзФайлаНастроек(?(ВыборОбъектаМножественный, ВыбранныеФайлы, ВыбранныеФайлы[0]));
...
Функция ЗначениеТаймаутаДляАсинхронногоШага(Знач КолСекунд) Экспорт
	...
	Возврат мин(Объект.ТаймаутДляАсинхронныхШагов,КолСекунд); 
КонецФункции
КонецФункции	 
...
&НаКлиенте
Процедура ЗагрузитьПеременныеКонтекстаИзФайлаНастроек(ПутьККаталогу = "")
	
	ЗагрузчикНастроек = Плагин("ЗагрузчикПользовательскихНастроек");
	
	Попытка
		ЗагрузчикНастроек.ЗагрузитьИзФайла(ПутьККаталогу);
	Исключение
...
&НаСервере
Процедура ДобавитьСнипетыСервер(КоллекцияШагов, ИмяФайла, ВерсияФайла, БылиОшибки, МассивВажныхСообщений)
	ТзнТаблицаИзвестныхStepDefinition = РеквизитФормыВЗначение("ТаблицаИзвестныхStepDefinition");//TODO ?? обойтись без РеквизитФормыВЗначение
...
Процедура ЗаполнитьПутьКRunSikulixЕслиОнПустой()
	...
	Если ПустаяСтрока(ПутьКRunSikulix)Тогда
		ЗагрузчикНастроек = Плагин("ЗагрузчикПользовательскихНастроек");
...
Функция ПрочитатьСтруктуруИзJSONФайлаСервер(VBParams, ПараметрыУФ)
	...
	Рез.Вставить("ТаймаутДляАсинхронныхШагов", 0.2);
...
Процедура СделатьСкриншотТекущегоОкна()
	...
	// TODO в иедеале надо делать безконтекстный вызов сервера.
	Sleep(1);
