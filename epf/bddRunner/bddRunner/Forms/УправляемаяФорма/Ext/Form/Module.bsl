...
&НаКлиенте
Перем Версия8312ИлиВыше;

&НаКлиенте
Перем Версия8313ИлиВыше;
...
Функция ПолучитьТабДокИзTestClientКакМассив(ЭлементФормы,ДопПараметры = Неопределено) Экспорт
	...
	Если НЕ ТекущийTestclientЭтоWebКлиент И ВерсияПриложенияБольшеИлиРавнаЧемЗаданная(СистемнаяИнформация.ВерсияПриложения, "8.3.8.0") Тогда
		...
		Пока Истина Цикл
			...
			Если КоличествоПопытокПолученияДвоичныхДанных > 10 Тогда
...
&НаКлиенте
Функция ВычислитьВнутреннееВыражение(Выражение) Экспорт
...
Процедура ПолучитьУжеСуществующиеСнипетыИзОбработок(Знач КаталогФич,
	...
	Для Каждого ОбъектФайл Из МассивФайлов Цикл
		...
		Если МассивИменФайловКоторыеСовпалиСКеш <> Неопределено Тогда
			МассивИменФайловКоторыеСовпалиСКеш.Добавить(УниверсальноеПолноеИмя);
			//МассивИменФайловКоторыеСовпалиСКеш.Добавить(ПреобразоватьПутьКФайлуКСтандарту(Файл.ПолноеИмя));
		КонецЕсли;
				...
				ИначеЕсли МассивОбъектов[ИдВМассиве - 1] = "FormButton" Тогда
					...
					Иначе
						...
						Если ДобавитьОбычнуюСтроку Тогда
							...
							Тогда
								ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " я жду закрытия окна '" + ТекущееОкно + "' в течение 120 секунд");
							КонецЕсли;
						КонецЕсли;
					КонецЕсли;
...
Процедура ЗаполнитьКонтекстСохраняемыйДляТестовоеПриложение(ТестовоеПриложение)
	...
	App = ТестовоеПриложение;
	Приложение = ТестовоеПриложение;
	MainWindow = ГлавноеОкноТестируемого;
	Ванесса  = ЭтаФорма;
КонецПроцедуры
